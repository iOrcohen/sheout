...
function CartDrawer({ open, onClose }) {
  ...
  useEffect(() => {
    const subscribe = async () => {
      const userSub = (await Auth.currentSession()).accessToken.payload.sub;
      console.log("Subscribedd ");

      const subscription = API.graphql(
        graphqlOperation(onCreateOrder, { owner: userSub })
      ).subscribe({
        next: ({ provider, value }) => {
          console.log("Subscription activate ");
